...
Then for the corresponding segment $ s(u) \in S $ there are $ \delta + 1 $ possible stub pairs including drawing the whole segment. We store the lengths of the possible stubs as node weights $ \ell_1(u), \dots, \ell_\delta(u) \in \mathbb{R}_+$ and assume that $ \ell_1(u),\dots,\ell_\delta(u) $ are sorted from shorter to longer stubs. We define  $ \ell_0(u) $ as the length of the whole segment $ s(u) $. 
