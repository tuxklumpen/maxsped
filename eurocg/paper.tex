...
Then for the corresponding segment $ s(u) \in S $ there are $ \delta + 1 $ possible stub pairs including drawing the whole segment, see Fig.~\ref{fig:tree}. We store the lengths of the possible stub pairs as node weights $ \ell_1(u), \dots, \ell_\delta(u) \in \mathbb{R}_+$ and assume that $ \ell_1(u),\dots,\ell_\delta(u) $ are sorted from shorter to longer stubs. We define  $ \ell_0(u) $ as the length of the whole segment $ s(u) $. 

\begin{figure}[tbp]
	\centering
 	\includegraphics{tree_illustration}
	\caption{A segment $s(u)$ with five intersecting segments and the different induced stub lengths. The boxed stub lengths are considered in $\solshort(u)$.} % The shaded areas are the stubs, $ p(u) $ is the parent of $ u $. The stubs in the boxed area contribute to $ \solshort(u) $.}
	\label{fig:tree}
\end{figure}
...
\]
%\end{align*}
Figure~\ref{fig:tree} highlights the stub lengths that contribute to $\solshort(u)$. We recursively define % for a vertex $ u $ and stub length $ \ell_i(u) $:
\begin{align}
	\label{rec:tree}
