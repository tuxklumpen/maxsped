...
\begin{proof}
	...
	The path itself consists of a chain of red-green segment pairs, where, as before, each pair can contribute an ink value of at most~7.
	...
	Conversely, assume that we have a SPED with ink $L$. 
	By construction, every red-green segment pair and all clause gadgets must contribute their respective maximum ink value.
	...
	For each clause, one segment must have full length and as argued before the path connecting that segment to a variable gadget must have all segments at even positions as stubs and all segments at odd positions as full segments. 
...
\end{lemma}
\begin{proof}
	Let $ G $, $ \Gamma $,  $ C = (V,E) $ with maximum degree $ k $ and $ \mathcal T = (\mathcal V,\mathcal E) $ as above and $ u \in \mathcal V $ a block vertex. Computing the value for the cycle itself $ \textit{cycle}(u) $ takes $ O(n_c) $ time by Bruckdorfer et al. with $ n_c $ being the number of vertices in the cycle. Now at every vertex $ v \in \textit{cycle}(u) $ there might be a set of up to $ k $ many children, which are all articulation points. Their values are already known and maximization over them can hence be done in $ O(k) $ time by the same argument as in Lemma~\ref{lem:compute_vertex}. Following we need $ O(n_ck) $ to compute the three values $ \solshort(u) $, $ \solmid(u) $ and $ \sollong(u) $.
\end{proof}
